    ...
    for t in range(max_len):
        ...
        for token in prev_tops:
            ...
            if token:
                ...
                p_nonblank = last.entries[token].p_nonblank \
                             * output_ctc[t, token[-1]]
...
class CTCModel(nn.Module):
    '''
    Base class for all model use CTC as loss function. This loss expects model
    return a sequence from image so that it can alignment between
    sequence from image, e.g. (N,S) and sequence from groundtruth, e.g. (N,T)
    '''
    def __init__(self, cnn, vocab, **config):
        ...
        if pool_type == 'adaptive_avg_pool':
            ...
            rand_tensor = torch.rand(1, 3, config['scale_height'],
...
class CTCModelTFEncoder(CTCModel):
    def __init__(self, cnn, vocab, **config):
        ...
        # use TransformerEncoder as decoder instead of e.g. LSTM
        decoder_layer = nn.TransformerEncoderLayer(d_model=cnn.n_features,
                                                   nhead=config['nhead'])
...
class CTCModelTF(CTCModel):
    def __init__(self, cnn, vocab, **config):
        super().__init__(cnn, vocab, **config)
        self.decoder = nn.Transformer(cnn.n_features,
                                      config['nhead'],
                                      config['encoder_nlayers'],
    ...
    def _forward_decode(self, images: torch.Tensor) -> torch.Tensor:
        ...
        images = self.decoder(images, images, tgt_mask=attn_mask)  # [S,B,C]
        images = images.transpose(0, 1)  # [B,S,C]
        outputs = self.character_distribution(images)  # [B,S,V]
        return outputs
